...
const datablockName = "SingleCityExample";

function SingleCityExample(props){
    // state variables
 ...
 useEffect(() => {
    ...
    //     )
    // }
  props.db.collection(`GeneralData`).doc(datablockName).onSnapshot((doc) => {
    ...
    //request functions
    const handleUpdate = () => {
        props.db.collection(`GeneralData`).doc(datablockName).set({
        datetime: new Date(),
    ...
    const handleDelete = () => {
      props.db.collection(`GeneralData`).doc(datablockName).delete().then(function() {
...
export default SingleCityExample